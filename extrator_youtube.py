...
print(f"Iniciando varredura no YouTube para o nicho: '{nicho_pesquisa}'...")

try:
    # 1. Busca os IDs
    request_busca = youtube.search().list(q=nicho_pesquisa, part='id', type='video', order='date', maxResults=max_resultados)
    resultado_busca = request_busca.execute()
    ids_videos = [item['id']['videoId'] for item in resultado_busca['items'] if 'videoId' in item['id']]

    # 2. Busca os detalhes e estatísticas
    request_stats = youtube.videos().list(part='statistics,snippet', id=','.join(ids_videos))
    ...
    if not df_yt.empty:
        ...
        print(f"\n🚀 Sucesso! Analisados {len(df_yt)} vídeos.")
    else:
